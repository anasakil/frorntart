...
export const updateProduct = createAsyncThunk(
    ...
    async ({ productId, productData, token }, { rejectWithValue }) => {
        console.log("Updating product with ID:", productId); // Log the product ID to ensure it's being passed
        if (!productId) {
            console.error("Attempted to update with undefined productId");
            return rejectWithValue('Product ID is undefined');
        }
        try {
            const response = await fetch(`http://localhost:3001/api/products/${productId}`, {
